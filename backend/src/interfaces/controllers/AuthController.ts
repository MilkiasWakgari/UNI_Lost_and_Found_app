...
export class AuthController {
  ...
  async requestPasswordReset(req: Request, res: Response) {
    try {
      ...
      await userRepo.update(user.id, { resetCode: code, resetCodeExpires: expires });
      // Return code in response (for local dev)
      res.json({ resetCode: code, message: 'Reset code generated. Use this code to reset your password.' });
    } catch (error: any) {
      res.status(500).json({ message: error?.message || 'Failed to generate reset code' });
    }
  }
  ...
  async resetPassword(req: Request, res: Response) {
    try {
      ...
      if (user.resetCode !== code || !user.resetCodeExpires || new Date(user.resetCodeExpires) < new Date()) {
        return res.status(400).json({ message: 'Invalid or expired code' });
      }
      // Update password and clear code
      const bcrypt = require('bcryptjs');
      const hashed = await bcrypt.hash(newPassword, 10);
      await userRepo.update(user.id, { password: hashed, resetCode: undefined, resetCodeExpires: undefined });
  ...
  async updateProfile(req: Request, res: Response) {
    try {
      ...
      const userRepo = new MongoUserRepository();
      const updatedUser = await userRepo.update(userId.toString(), { fullName, email });
      if (updatedUser) {
