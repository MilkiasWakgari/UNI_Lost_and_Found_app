...
export class ItemController {
    ...
    async deleteItem(req: Request, res: Response, next?: NextFunction) {
        try {
            ...
            if (req.user && (req.user._id.equals(item.foundBy) || req.user.role === 'admin')) {
    ...
    async getRecentItems(req: Request, res: Response, next?: NextFunction) {
        try {
            ...
            res.status(500).json({ message: error?.message || 'Failed to fetch recent items' });
        }
    }
    ...
    async getTodayItems(req: Request, res: Response, next?: NextFunction) {
        try {
            ...
            res.status(500).json({ message: error?.message || 'Failed to fetch today\'s items' });
        }
    }
    ...
    async markAsReturned(req: Request, res: Response, next?: NextFunction) {
        try {
            ...
            res.status(400).json({ message: error?.message || 'Failed to mark item as returned' });
        }
    }
} 